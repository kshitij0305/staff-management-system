...
/** Which roles each role is allowed to create (always as their own direct reports, except Chairman). */
export const CREATABLE_ROLES: Record<Role, Role[]> = {
  CHAIRMAN: [Role.NATIONAL_HEAD, Role.CSM, Role.ASM, Role.CPE],
...
/** Chairman and National Head see the whole company. */
export function seesEverything(role: Role): boolean {
  return role === Role.CHAIRMAN || role === Role.NATIONAL_HEAD;
}
...
export function canManageUser(
  ...
  if (session.role === Role.CHAIRMAN) return true;
